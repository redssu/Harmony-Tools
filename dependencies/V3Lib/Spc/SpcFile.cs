...
namespace V3Lib.Spc
{
    public class SpcFile
    {
        ...
        public byte[] Unknown1 = new byte[0x24];
        public int Unknown2;
        ...
        public void Load(string spcPath)
        {
            ...
            if (magic != "CPS.")
            {
                ...
                throw new InvalidDataException(
                    $"Invalid magic number, expected \"CPS.\" but got \"{magic}\"."
                );
            }
            ...
            if (tableHeader != "Root")
            {
                ...
                //return;
                throw new InvalidDataException(
                    $"Invalid file table header, expected \"Root\" but got \"{tableHeader}\"."
                );
            }
            ...
            for (int i = 0; i < fileCount; ++i)
            {
                ...
                subfile.Name = Encoding
                    .GetEncoding("shift-jis")
                    .GetString(reader.ReadBytes(nameLength));
            ...
            foreach (SpcSubfile subfile in Subfiles)
            {
                if (filename == subfile.Name)
                {
                    ...
                    using FileStream output = new FileStream(
                        outputLocation + Path.DirectorySeparatorChar + filename,
                        FileMode.Create
                    );
        ...
        public void InsertSubfile(string filename, bool compress = true)
        {
            ...
            if (!insertInfo.Exists)
            {
                ...
                throw new FileNotFoundException(
                    $"Source file \"{insertInfo.FullName}\" does not exist."
                );
            }
