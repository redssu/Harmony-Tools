...
namespace V3Lib.Wrd
{
    ...
    public class WrdFile
    {
        ...
        public void Load(string wrdPath)
        {
            ...
            if (stringsPtr != 0)
            {
                using BinaryReader stringReader = new BinaryReader(
                    reader.BaseStream,
                    Encoding.Unicode,
                    true
                );
            ...
            while (reader.BaseStream.Position + 1 < localBranchDataPtr)
            {
                ...
                while (reader.BaseStream.Position + 1 < localBranchDataPtr)
                {
                    ...
                    byte argType = WrdCommandHelper.ArgTypeLists[op][
                        argNumber % WrdCommandHelper.ArgTypeLists[op].Count
                    ];
                    switch (argType)
                    {
        ...
        public void Save(string wrdPath)
        {
            ...
            List<(ushort ID, ushort Offset)> localBranchData =
                new List<(ushort ID, ushort Offset)>();
            List<string> labelNames = new List<string>();
            List<string> parameters = new List<string>();
            ...
            foreach (WrdCommand command in Commands)
            {
                ...
                switch (command.Opcode)
                {
                    ...
                    case "LBN":
                        ...
                        localBranchData.Add(
                            (ushort.Parse(command.Arguments[0]), (ushort)commandData.Length)
                        );
                        break;
                }
            ...
            if (!UsesExternalStrings && stringCount > 0)
            {
                ...
                writer.BaseStream.Seek(0, SeekOrigin.End);
                using BinaryWriter stringWriter = new BinaryWriter(
                    writer.BaseStream,
                    Encoding.Unicode,
                    true
                );
